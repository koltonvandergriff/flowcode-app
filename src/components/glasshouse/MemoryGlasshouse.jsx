// Glasshouse Memory page. Page header + glassy frame around the existing
// MemoryPanel (in embedded mode). All memory functionality intact —
// cosmos, categories, search, semantic search, AI categorize, embeddings,
// trash, wiki-links, backlinks, all wired through the existing component.
...
export default function MemoryGlasshouse({ onClose }) {
  return (
    <div style={s.root}>
      <div style={s.head}>
        <div style={s.headTextWrap}>
          <h1 style={s.h1}>Memory</h1>
          <p style={s.sub}>Force-graph cosmos · AI categorization · semantic search · MCP-accessible from every terminal.</p>
        </div>
        <div style={s.headChips}>
          <span style={s.chip}><span style={s.dot} /> Live</span>
          <span style={s.chipMute}>⌘ M to focus</span>
        </div>
      </div>

      <div style={s.frame}>
        <ErrorBoundary name="Glasshouse · Memory">
          <MemoryPanel open={true} embedded={true} onToggle={onClose} />
        </ErrorBoundary>
      </div>
    </div>
  );
